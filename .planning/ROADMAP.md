...
**Key tasks:**
- Add FirewallStats, LoadScript, RemoveScript, InjectScore message and RPC definitions to `api/admin.proto`
- Run `generate.sh` to regenerate Go stubs
- Implement handlers in `internal/admin/` delegating to `internal/firewalld/` methods
- Wire new gRPC server into startup path alongside existing HTTP admin
- Add unit tests for each RPC handler
